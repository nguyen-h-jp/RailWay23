...
//		test();
	...
	static void Question5(Scanner sc) {
		...
		System.out.println("ID: " + ac.accountID + 
							...
							", Position: " + ac.position.positionName);
		...
		if(indexAccount < 0 || indexGroup < 0) {
			...
			for (int i = 0; i < arr.length; i++) {
				if (arr[i].userName.equals(inputUserName)) {
					...
					System.out.println("Bạn vừa thêm account: " + arr[indexAccount].userName + " vào group: " + arr[indexAccount].groups[0].groupName);	
				}
			}
		}	
	}
	...
	static void addAccountToRandomGroup(Account[] arr, Group[] gr, Scanner sc) {
		Random random = new Random();
		System.out.println("Nhập vào username: ");
		String userName = sc.nextLine();
		userName = sc.nextLine();
		int r = random.nextInt(gr.length);
		for (int j = 0; j < arr.length; j++) {
			if (arr[j].userName.equals(userName)) {
				Group[] groupAdd = {gr[r]};
				arr[j].groups = groupAdd;
				System.out.println("Bạn vừa thêm account: " + arr[j].userName + " vào group: " + arr[j].groups[0].groupName);	
			}
		}
	}
	...
	static void Question11(Account[] arr, Group[] gr, Scanner sc) {
		...
		while(!isDoneFlag) {
			if(input == 1) {
				...
				addAccountToRandomGroup(arr,gr,sc);
				isDoneFlag = true;
			}else {
				System.out.println("Mời bạn nhập lại");
				isDoneFlag = false;	
