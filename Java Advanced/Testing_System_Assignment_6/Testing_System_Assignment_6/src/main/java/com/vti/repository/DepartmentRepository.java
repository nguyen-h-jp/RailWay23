...
public class DepartmentRepository implements IDepartmentRepository {
	...
	public List<Department> getAllDepartments(Integer pageNumber, Integer pageSize, String name, String sortType, String filtername) {
		...
		try {
			...
			// sort by name (tăng hoặc giảm theo tên)
			
			if(name != null && sortType != null) {
				
				criteria.addOrder(sortType.equals("asc") ? Order.asc(name) : Order.desc(name));
				
			}
			
			// filter
			if(filtername != null) {
				criteria.add(Restrictions.ilike("departmentName", "%" + filtername + "%"));
			}
